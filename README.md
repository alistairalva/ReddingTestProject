...
- Auth: plugin accepts `Authorization: Bearer dev-secret-token-CHANGE-THIS` or header `X-Auth-Token`. This is a simulation for the test; change token in `test-resources-plugin.php` constant if you want. Make sure to match it in the frontend/.env file
...
- Toggle "Authenticated" in UI to have the frontend include the simulated auth header.
